...
plot_tab <- function(df_cv_preds_and_coefs,
  ...
  df_tab_prep1 <- df_coef_final %>%
    ...
    arrange(var, min) %>%
    mutate(gr = ifelse(! is.na(x) | ! is.na(lag(x)), 1, 0),
           gr = cumsum(gr),
    ...
    mutate(min_adj = ifelse(is.na(x) & ! is.na(lag(max)), lag(max), min),
           max_adj = ifelse(is.na(x) & ! is.na(lead(min)), lead(min), max),
           zero_size_bin = min_adj == max_adj,
           keep = ! zero_size_bin | ! is.na(x) | data_type == "cat") %>%
    filter(keep) %>%
    ungroup() %>%
    select(name_pretty, min = min_adj, max = max_adj, x, ci95_low, ci95_high,
  ...
  tb <- df_tab_prep2 %>%
    ...
    tab_style(
      ...
      locations = list(cells_body(c("N", "N with Findings", "Odds Ratio (CI95)")),
